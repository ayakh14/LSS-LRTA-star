...
class AStar:
  ...
  def __init__(self, graph, start_position, target, lookahead = 0):
    self.graph = graph
  ...
  def calculate_g_distance(self, parent, child):
    """
    ...
    for neighbor in parent.neighbors:
      if neighbor[0] == child:
        print(f"parent distance from the start:{parent.distance_from_start}, child:{child}")

        distance = parent.distance_from_start + neighbor[1]
        print("parent:{}, distance:{}, parent distance form start:{}, neighbor[1]:{}, child sistance from the start:{}".format(parent, distance, parent.distance_from_start, neighbor[1], child.distance_from_start))
        print(f"child:{child}, parent:{child.parent}")
        if distance < child.distance_from_start:
          # child.parent = parent
          child.add_parent(parent)
          return distance
        print(f"child.distance_from_start:{child.distance_from_start}")
        return child.distance_from_start


  def calculate_f_value(self, parent, child, target):
    """
      ...
      -------
        int
    """
    return self.calculate_g_distance(parent, child) + self.manhattan_distance(child, target)
  ...
  def calculate_path(self, target_node):
    ...
    while current is not None:
      # current.value = to the name of the node
      # path.append(current.value)
        path.append(current.value)
        # print(f"current node{current}, value:{current.value}, current parent:{current.parent}")
        current = current.parent 
        
    return path  # Return rev
  ...
  def search(self):
    ...
    self.start.f_value = self.start.heuristic_value + self.start.distance_from_start
    ...
    while True:
      ...
      if (self.expansion != self.lookahead):
      ...
      elif (self.expansion == self.lookahead): 
      ...
      if len(new_nodes) > 0:
        for new_node in new_nodes:
          # new_node.g_value = self.
          # new_node.heuristic_value = self.manhattan_distance(new_node, self.target)
          new_node.f_value = self.calculate_f_value(selected_node, new_node, self.target)
